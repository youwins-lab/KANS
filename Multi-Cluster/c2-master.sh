...
echo "[TASK 1] Initial Kubernetes - Skip Kube-proxy , Pod CIDR 10.20.0.0/16 , API Server 192.168.20.10"
kubeadm init --skip-phases=addon/kube-proxy --token 123456.1234567890123456 --token-ttl 0 --pod-network-cidr=10.20.0.0/16 --apiserver-advertise-address=192.168.20.10 >/dev/null 2>&1
...
helm install cilium cilium/cilium --version 1.11.2 --namespace kube-system --set k8sServiceHost=192.168.20.10 --set k8sServicePort=6443 --set debug.enabled=true --set autoDirectNodeRoutes=true --set endpointRoutes.enabled=true --set hubble.relay.enabled=true --set hubble.ui.enabled=true --set ipam.mode=kubernetes --set k8s.requireIPv4PodCIDR=true --set kubeProxyReplacement=strict --set ipv4NativeRoutingCIDR=192.168.20.0/24 --set tunnel=disabled --set operator.replicas=1 --set egressGateway.enabled=true --set enableIPv4Masquerade=true --set bpf.masquerade=true --set loadBalancer.mode=dsr --set bandwidthManager=true --set prometheus.enabled=true --set operator.prometheus.enabled=true --set hubble.metrics.enabled="{dns:query;ignoreAAAA,drop,tcp,flow,icmp,http}" --set hostServices.enabled=true --set bpf.hostRouting=true >/dev/null 2>&1
